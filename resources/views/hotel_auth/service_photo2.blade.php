	...
	<div style="max-width: 850px;max-height: 100px;height: 100px;padding: 10px;margin-top:10px;">
		<div style="max-height: 100px;text-align: right;">
			<form id="photo_form" method="post" enctype="multipart/form-data" action="../service_photo_upload/{{$ServiceID}}">
				{{ csrf_field() }}
			  <span style="margin-right: 50px;font-weight: bold;">{{$ServiceName->service_name}}</span>
		...
		<div style="max-height: 100px;">
			費用：
			<input id="cost" name="cost" type="text" class="" placeholder="免費或付費 (無則免填.網站不顯示)" value="@if(!empty($ServiceInfo)) {{$ServiceInfo->cost}} @endif" onblur="chgInfo()" style="width:40%;">
			開放時段：
			<input id="period" name="period" type="text" class="" placeholder="24小時或上午9點至下午10點(無
則免填.網站不顯示)" value="@if(!empty($ServiceInfo)) {{$ServiceInfo->period}} @endif" onblur="chgInfo()" style="width:40%;">
		</div>
		<div style="max-height: 100px;">
			說明：
			<input type="text" id="comm" name="comm" class="" placeholder="付費說明及使用須知(無則免填.網站不顯示)" aria-label="Small" aria-describedby="inputGroup-sizing-sm" value="@if(!empty($ServiceInfo)) {{$ServiceInfo->comm}} @endif" style="width:91%;margin-top: 5px;" onblur="chgInfo()">
		</div>
	</div>
</div>
 </div>
